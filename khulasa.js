(function() {
    ...
    Khulasa.VERSION = '1.0.0';

    var self = this.Khulasa;

    var Summarizer = self.Summarizer = {};

    Summarizer.VERSION = '1.0.0';


    self.SummaryItem = function(text, isHighlighted) {
        this.itemText = text;
    ...
    Summarizer.summarize = function(title, items, callback) {
        var hasTitle = true;
        ...
        //new Khulasa2().summarize(content, function(title, summary, failure){console.log(summary)});

        this.knwljsScan(content);
    ...
    Summarizer.getSortedSentences = function(content, n, callback) {
    ...
    Summarizer.knwljsScan = function(content) {
        var knwljs = new Knwl();
    ...
    self.show = function() {
        ...
        document.getElementById('khulasa-overlay').style.display = "block";
        document.getElementById('khulasa-overlay-content').style.display = "block";
        ...
        $(document.body).find("p, h1, h2, h3, h4, h5, h6").each(function() {
            var isHighlighted = false;
            if (typeof $(this).attr('data-khulasa') !== "undefined") {
                if ($(this).attr('data-khulasa') == 'true') {
                    highlights++;
                    isHighlighted = true;
                } else {
                    return;
            ...
            contentLength += $(this).text().length;
            summaryItems.push(new self.SummaryItem($(this).text(), isHighlighted));
        });

        if (summaryItems.length > 0) {
            console.log("Found: " + summaryItems.length + " items (" + highlights + " highlights)");
            self.Summarizer.summarize(title, summaryItems, function(err, summary) {
                if (err) {
                    console.log("Something went wrong!");
                    return;
                }
                $("#khulasa-output").html(summary);
                console.log("Summary: " + summary);
                console.log("Original Length: " + (title.length + contentLength));
                console.log("Summary Length: " + summary.length);
                console.log("Summary Ratio: " + (100 - (100 * (summary.length / (title.length + contentLength)))));
            });
        }
    }
    ...
    self.hide = function()
    {
        document.getElementById('khulasa-overlay').style.display = "none";
        document.getElementById('khulasa-overlay-content').style.display = "none";
    }
}).call(this);
